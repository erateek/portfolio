...
import { ArrowRight, Calendar, Clock, Share2, ArrowUpRight, Star, TrendingUp, Zap, Award } from 'lucide-react';
...
export async function generateMetadata({ params }) {
  ...
  if (!post) {
    return {
      title: 'المقال غير موجود | مدونة إراتيك',
    };
  }

  return {
    title: `${post.title} | مدونة إراتيك`,
...
export default async function BlogPost({ params }) {
  ...
  const recommendedPosts = await getRecommendedPosts(slug, 'ar', 3);
  ...
  if (badgeType === 'featured') {
      SpecialBadge = { icon: Star, text: 'مميز', color: 'text-yellow-500 bg-yellow-500/10 border-yellow-500/20' };
  } else if (badgeType === 'trending') {
      SpecialBadge = { icon: TrendingUp, text: 'رائج', color: 'text-blue-500 bg-blue-500/10 border-blue-500/20' };
  } else if (badgeType === 'new') {
      SpecialBadge = { icon: Zap, text: 'جديد', color: 'text-purple-500 bg-purple-500/10 border-purple-500/20' };
  } else if (badgeType === 'editorsChoice') {
      SpecialBadge = { icon: Award, text: 'اختيار المحرر', color: 'text-rose-500 bg-rose-500/10 border-rose-500/20' };
  ...
  const ptComponents = {
    ...
    block: {
      blockquote: ({ children }) => (
        <blockquote className="border-r-4 border-primary bg-surface py-6 px-8 rounded-3xl not-italic shadow-sm my-8">
          {children}
        </blockquote>
      )
    }
        ...
        <section className="relative pt-24 pb-8 md:pt-36 md:pb-16 px-4 md:px-6">
           ...
           <div className="container mx-auto max-w-4xl relative z-10">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
              <Link
                href="/blog"
                className="inline-flex items-center gap-2 text-sm font-bold text-muted hover:text-primary transition-colors group p-2 -mr-2"
              >
            ...
            <div className="flex flex-wrap items-center gap-y-4 gap-x-8 text-sm border-b border-foreground/5 pb-8 mb-8">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="text-muted text-xs">فريق التحرير</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 text-muted bg-surface px-3 py-2 rounded-lg border border-foreground/5">
                ...
                <time dateTime={post.publishDate}>
                  {new Date(post.publishDate).toLocaleDateString("ar-EG", {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                  })}
             ...
             <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-[2.5rem] p-8 md:p-12 relative overflow-hidden text-center shadow-2xl shadow-slate-900/20">
                ...
                <div className="relative z-10 text-white">
                  ...
                  <p className="text-slate-300 text-base md:text-lg mb-8 max-w-lg mx-auto leading-relaxed">
                    شارك هذه الأفكار مع شبكتك وابدأ النقاش.
                  </p>
            ...
            <div className="container mx-auto max-w-6xl">
               <div className="flex flex-col md:flex-row items-end justify-between gap-4 mb-12">
                  <div>
                     <h2 className="text-3xl md:text-4xl font-black text-foreground mb-2">
                       {post.locale === 'ar' ? 'اقرأ أيضاً' : 'Recommended for you'}
                     </h2>
                     <p className="text-muted text-lg">
                       {post.locale === 'ar' ? 'مقالات قد تثير اهتمامك' : 'Articles you might find interesting'}
                     </p>
                  </div>
                  <Link href="/blog" className="text-primary font-bold hover:underline flex items-center gap-2">
                     {post.locale === 'ar' ? 'عرض كل المقالات' : 'View all articles'}
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {recommendedPosts.map((recPost) => (
                     <Link href={`/blog/${recPost.slug}`} key={recPost.slug} className="group flex flex-col h-full bg-surface rounded-3xl border border-foreground/5 overflow-hidden hover:shadow-2xl hover:shadow-primary/5 hover:-translate-y-2 transition-all duration-300">
                        ...
                        <div className="p-6 flex flex-col flex-1">
                           <div className="flex items-center gap-2 mb-3">
                              <span className="px-3 py-1 bg-primary/10 text-primary text-xs font-bold rounded-lg border border-primary/20">
                                 {recPost.category || 'عام'}
                              </span>
                              <span className="text-xs text-muted flex items-center gap-1">
                                 <Calendar className="w-3 h-3" />
                                 {new Date(recPost.publishDate).toLocaleDateString(post.locale === 'ar' ? 'ar-EG' : 'en-US')}
                              </span>
                           </div>
                           ...
                           <div className="flex items-center gap-2 text-primary font-bold text-sm mt-auto">
                              <span>{post.locale === 'ar' ? 'اقرأ المزيد' : 'Read more'}</span>
